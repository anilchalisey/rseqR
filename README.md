...
`DESCRIPTION` file. These may be installed manually, otherwise the
...
`gage` package on the objects contained within `DE-results.rda`. Loading
this RDA file will populate the environment with the objects `DE_limma`,
`DE_edger`, and `DE_deseq2`. Each of these is a list with the results of
pairwise differential expression analyses. In the example used abvoe,
the three conditions were `WT`, `KO1`, and `KO2`; thus, the three
comparisons are `KO1-WT`, `KO2-WT`, and `KO2-KO1`. To perform pathway
analysis on the differential genes identified by DESeq2 for the first
comparison, the following command may be used:
